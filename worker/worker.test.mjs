...
export function fakeEnvironment() {
  return {
    SITE_PASSWORD: "Synthetic-server-password-9284",
    DATA_REPO_TOKEN: "github_pat_" + "x".repeat(48),
    SESSION_SECRET: "synthetic-session-secret-" + "s".repeat(40),
    ...
    ASSETS: {
      fetch: async () =>
        new Response("<!doctype html><title>Public shell</title>", {
...
function request(
  path,
  { method = "GET", cookie, body, from = origin, headers = {} } = {},
) {
  if (method === "PUT" && body && typeof body === "object")
    body = { clientVersion: 2, ...body };
  const h = { ...headers };
  if (cookie) h.Cookie = cookie;
  if (method !== "GET" && method !== "HEAD") {
    if (from !== null) h.Origin = from;
...
async function login(env) {
  const r = await worker.fetch(
    request("/api/login", {
      method: "POST",
      body: { password: env.SITE_PASSWORD },
    }),
    env,
  );
...
function githubMock(env) {
  ...
  const fetcher = async (url, options = {}) => {
    ...
    assert.equal(
      u.pathname,
      "/repos/test-owner/private-data/contents/data/team.json",
    );
    if (options.method === "PUT") {
      if (readOnly) return response(403, {});
      const update = JSON.parse(options.body);
      if ((exists && update.sha !== sha) || (!exists && update.sha))
        return response(409, {});
      state = JSON.parse(
    ...
    return response(200, {
      ...
      content: Buffer.from(
        corrupt ? "not json" : JSON.stringify(state),
      ).toString("base64"),
    });
  };
  return {
...
test("server session, private archive, validation and credential boundary", async (t) => {
  ...
  assert.equal(
    (await worker.fetch(request("/worker/index.mjs"), env)).status,
    404,
  );
  assert.equal((await worker.fetch(request("/.dev.vars"), env)).status, 404);
  assert.equal((await worker.fetch(request("/api/archive"), env)).status, 401);
  assert.equal(mock.calls.length, 0);
  const anonymous = await worker.fetch(request("/api/session"), env);
  ...
  assert.equal(mock.calls.length, 0);
  const loggedIn = await worker.fetch(
    request("/api/login", {
      method: "POST",
      body: { password: env.SITE_PASSWORD },
    }),
  ...
  const cookie = sessionCookie(loggedIn),
    payload = await loggedIn.json();
  assert.equal(payload.authenticated, true);
  assert(!JSON.stringify(payload).includes(env.DATA_REPO_TOKEN));
  assert(!setCookie.includes(env.SITE_PASSWORD));
  ...
  assert(loaded.headers.get("Cache-Control").includes("no-store"));
  const archive = await loaded.json();
  assert.deepEqual(archive.state, blank());
  ...
  const saved = await worker.fetch(
    request("/api/archive", {
      method: "PUT",
      cookie,
      body: { state: next, sha: archive.sha, path: "unauthorized.json" },
    }),
    env,
  );
  assert.equal(saved.status, 200);
  const savedData = await saved.json();
  assert.equal(savedData.state.members.length, 1);
  assert(savedData.state.updatedAt);
  assert.equal(mock.state.members[0].memo, "가상 기록");
  const conflict = await worker.fetch(
    request("/api/archive", {
      method: "PUT",
      cookie,
      body: { state: next, sha: archive.sha },
    }),
    env,
  );
  assert.equal(conflict.status, 409);
  assert.equal(mock.state.members.length, 1);
  const removed = await worker.fetch(
    request("/api/archive", {
      method: "PUT",
      cookie,
      body: { state: blank(), sha: savedData.sha },
    }),
    env,
  );
  assert.equal(removed.status, 200);
  assert.equal(mock.state.members.length, 0);
  const malformed = structuredClone(next);
  malformed.members[0].memo = "x".repeat(1501);
    ...
    (
      await worker.fetch(
        request("/api/archive", {
          method: "PUT",
          cookie,
          body: { state: malformed, sha: mock.sha },
        }),
        env,
      )
    ).status,
  ...
  assert.equal(
    (
      await worker.fetch(
        request("/api/archive", { method: "DELETE", cookie }),
        env,
      )
    ).status,
    405,
  ...
  assert.equal(
    (
      await worker.fetch(
        request("/api/login", { method: "POST", body: "x".repeat(4100) }),
        env,
      )
    ).status,
    413,
  ...
  assert.equal(
    (
      await worker.fetch(
        request("/api/login", { method: "POST", body: "bad-json" }),
        env,
      )
    ).status,
    400,
  ...
  assert.equal(
    (await worker.fetch(request("/api/archive", { cookie }), env)).status,
    503,
  );
  ...
  mock.setRedirected(true);
  const redirect = await worker.fetch(request("/api/archive", { cookie }), env);
  assert.equal(redirect.status, 502);
  assert.equal((await redirect.json()).error.code, "UPSTREAM_REDIRECT");
  mock.setRedirected(false);
  mock.setCorrupt(true);
  assert.equal(
    (await worker.fetch(request("/api/archive", { cookie }), env)).status,
    502,
  );
  mock.setCorrupt(false);
  mock.setExists(false);
  const missing = await worker.fetch(request("/api/archive", { cookie }), env);
  assert.equal(missing.status, 404);
  assert.equal((await missing.json()).error.code, "ARCHIVE_NOT_INITIALIZED");
  const init = await worker.fetch(
    request("/api/archive", {
      method: "PUT",
      cookie,
      body: { state: blank(), sha: null },
    }),
    env,
  );
  assert.equal(init.status, 200);
  const logout = await worker.fetch(
    request("/api/logout", { method: "POST", cookie, body: {} }),
    env,
  );
...
test("CSRF, tampered/expired sessions, password changes and login protection", async () => {
  ...
  for (const path of ["/api/login", "/api/logout", "/api/archive"]) {
    assert.equal(
      (
        await worker.fetch(
          request(path, {
            method: path === "/api/archive" ? "PUT" : "POST",
            cookie,
    ...
    assert.equal(
      (
        await worker.fetch(
          request(path, { method: "POST", cookie, from: null, body: {} }),
          env,
        )
      ).status,
      403,
  ...
  assert.equal(
    (
      await worker.fetch(
        request("/api/archive", { cookie: parts.join(".") }),
        env,
      )
    ).status,
    401,
  );
  const other = new Request("https://another-site.test/api/archive", {
    headers: { Cookie: cookie },
  });
  assert.equal((await worker.fetch(other, env)).status, 401);
  const changed = { ...env, SITE_PASSWORD: "A-different-valid-password-3928" };
  assert.equal(
    (await worker.fetch(request("/api/archive", { cookie }), changed)).status,
    401,
  );
  const now = Date.now;
  try {
    Date.now = () => now() + 13 * 3600000;
    assert.equal(
      (await worker.fetch(request("/api/archive", { cookie }), env)).status,
      401,
    );
  ...
  const limit = await worker.fetch(
    request("/api/login", {
      method: "POST",
      body: { password: env.SITE_PASSWORD },
    }),
  ...
  assert.equal(
    (await worker.fetch(request("/api/archive", { cookie }), apiLimited))
      .status,
    429,
  );
  assert.equal(
    (await worker.fetch(request("/api/session"), { ...env, SITE_PASSWORD: "" }))
      .status,
    503,
  );
  assert.equal(
    (
      await worker.fetch(request("/api/session"), {
        ...env,
        DATA_REPO_TOKEN: "ghp_" + "a".repeat(36),
      })
    ).status,
    503,
  );
...
test("administrator-selected five-character password is supported", async () => {
  ...
  const restored = await worker.fetch(
    request("/api/session", { cookie: sessionCookie(good) }),
    env,
  );
  assert.equal((await restored.json()).authenticated, true);
  const wrong = await worker.fetch(
    request("/api/login", { method: "POST", body: { password: "Qa7x3" } }),
    env,
  );
  assert.equal(wrong.status, 401);
