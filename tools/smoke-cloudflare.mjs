...
async function main() {
  ...
  try {
    ...
    for (const payload of [login.json, session.json, archive.json])
      if (
  ...
  console.log(
    "Live password login, session cookie, anonymous denial and archive read checks completed. No production records were modified.",
  );
}
main().catch((error) => {
