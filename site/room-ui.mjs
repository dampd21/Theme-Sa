...
export function createRoomUI(H) {
  ...
  function mazeSVG(m, path = walk) {
    return `<svg viewBox="0 0 300 300" class="room-maze-map" role="img" aria-label="공동 미로 전체 지도"><rect width="300" height="300" fill="#c7d7b6"/>${m.cells.map((v, i) => `<rect x="${(i % 15) * 20 + 1}" y="${Math.floor(i / 15) * 20 + 1}" width="18" height="18" fill="${v ? "#223e33" : fixedCell(i) ? "#dfcf97" : "#aec1a1"}"/>`).join("")}${path.length > 1 ? `<path d="${path.map((p, i) => (i ? "L" : "M") + ((p % 15) * 20 + 10) + " " + (Math.floor(p / 15) * 20 + 10)).join(" ")}" stroke="#66858b" stroke-width="4" fill="none"/>` : ""}<text x="10" y="15" font-size="14" text-anchor="middle" fill="#23402e">入</text><text x="290" y="295" font-size="14" text-anchor="middle" fill="#23402e">出</text><circle cx="${(path.at(-1) % 15) * 20 + 10}" cy="${Math.floor(path.at(-1) / 15) * 20 + 10}" r="6" fill="#81516b" stroke="#fff" stroke-width="2"/></svg>`;
  }
  function maze() {
    const m = data.maze,
